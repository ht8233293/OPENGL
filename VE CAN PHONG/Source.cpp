...
void table()
{
    ...
    glPushMatrix();
    glTranslatef((table_width - leg_width) / 2 + 2.1, (table_length - leg_length) / 2 - 5, (table_height - leg_length) / 2 + 4.5); //leg4
    glScalef(leg_width, leg_length, leg_height);
    glTranslatef(-1, -1, -1);
    cube(0.53, 0.39, 0.28);
    glPopMatrix();
...
void display(void)
{
    ...
    glLoadIdentity();
    gluLookAt(eyeX, eyeY, eyeZ, lookX, lookY, lookZ, 0, 2, 0); //eyeX,eyeY,eyeZ //5,10,-20
